...
//import CommentSection from '../components/comentarios';
const RecetaGuardada = ({ route }) => {
  ...
  const guardarLike = async () => {
    ...
    if (user) {
      ...
      const recetaRef = firestore().collection("recetario").doc(encodedUrl);
      ...
      try {
        ...
        ToastAndroid.show('Me gusta', ToastAndroid.SHORT);
